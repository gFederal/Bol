...
local SCRIPT_NAME = "Federal Smite"
local SCRIPT_AUTHOR = "xFederaL"
local version = "1.0"
...
if myHero:GetSpellData(SUMMONER_1).name:find("SummonerSmite") then Smite = SUMMONER_1 
	elseif myHero:GetSpellData(SUMMONER_2).name:find("SummonerSmite") then Smite = SUMMONER_2 
	end

if Smite ~= nil then 
	DelayAction(function() PrintChat("<font color='#FF0000'>Federal Info </font>".."<font color='#00BFFF'><b>Smite found.</b></font>") end, 4.0) 
	DelayAction(function() PrintChat("<font color='#FF0000'>Federal Smite "..version.." </font>".."<font color='#00BFFF'><b>Loaded.</b></font>")  end, 4.0)
else
	DelayAction(function() PrintChat("<font color='#FF0000'>[Federal Info] </font>".."<font color='#00BFFF'>Smite not found.</font>")  end, 4.0) 
	return 
end
...
--Credits SxTeam  
 local ToUpdate = {}
    ...
    ToUpdate.VersionPath = "/gFederal/Bol/master/Federal%20Smite.version"
    ToUpdate.ScriptPath =  "/gFederal/Bol/master/Federal%20Smite.lua"
    ToUpdate.SavePath = SCRIPT_PATH.."/Federal Smite.lua"
...
function MenuInit()
	Menu = scriptConfig("Federal Smite", "Federal Smite")
	...
	Menu.permaShowEdit(IDPerma, "lText", "Federal Smite Active")
	Menu.permaShowEdit(IDPerma, "lTextColor", 0xFF00FF00)
	IDPerma = Menu:permaShow("UseSpells")
	Menu.permaShowEdit(IDPerma, "lText", "Federal Smite - Use Spells")
	Menu.permaShowEdit(IDPerma, "lTextColor", 0xFF00FF00)
end
    	...
    	if not jminion.dead and jminion.visible and ValidTarget(jminion, 560) then
      		...
      		if Menu.smite[name] then
       			...
       			if SpellReady(SMITESLOT) and GetDlinaForSmiteDraw(myHero, jminion) <= 560 then
        			...
        			DrawText("Progresso do Smite - "..SmiteProcess.."%", 25, posMinion.x - GetTextArea("Progresso do Smite - "..SmiteProcess.."%", 25).x/2, posMinion.y, ARGB(255,255,0,0))
       			end
       		end
     		end
   		end
